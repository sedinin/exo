...
aquire(Timeout) 
  when is_integer(Timeout); 
       Timeout =:= infinity ->
    Ref = make_ref(),
    gen_server:cast(?SERVER,{aquire, Ref, Timeout, self()}),
    receive
	Reply -> Reply
    after Timeout + 1000 ->
	    {error, not_available}
...
%% @end
%%--------------------------------------------------------------------
-spec aquire_async(Ref::term(), Timeout::timeout()) -> 
			  {ok, Ref::term()} | 
			  {error, Error::atom()}.

aquire_async(Ref, Timeout) 
  when is_integer(Timeout); 
       Timeout =:= infinity ->
    gen_server:cast(?SERVER,{aquire, Ref, Timeout, self()}).
...
init(Args) ->
    ...
    ReservedPorts = max(0.1 * MaxPorts, ?RESERVED_PORTS),
    MaxFds = max_fds(),
    ReservedFds = max(0.1 * MaxFds, ?RESERVED_FDS),
    Avail =  min(MaxPorts - ReservedPorts, MaxFds - ReservedFds),
    {ok, #ctx {state = up, available = Avail}}.
...
%%--------------------------------------------------------------------
-type cast_msg()::
	{aquire, Ref::term(), Timeout::timeout(), Pid::pid()} |
...
handle_info({timeout, _Timer, {aquire, _Pid, _Ref}} = I, 
	    Ctx=#ctx {available = Avail}) 
when Avail > 0 ->
    lager:debug("info ~p.", [I]),
    {noreply, handle_avail_timeout(I, Ctx)};

handle_info({timeout, _Timer, {aquire, _Pid, _Ref}} = I, Ctx) ->
    lager:debug("info ~p.", [I]),
...
%% Internal functions
%%--------------------------------------------------------------------
handle_aquire({aquire, Ref, _Timeout, Pid}, 
...
handle_aquire({aquire, Ref, Timeout, Pid}, 
    ...
    NewUsers = supervise(Pid, Users),
    Timer = erlang:start_timer(Timeout, self(), {aquire, Pid, Ref}),
    Ctx#ctx {users = NewUsers, waiting = Waiting ++ [{{Pid, Ref}, Timer}]}.

handle_release({release, _Ref, Pid}, Ctx=#ctx {waiting = Waiting}) ->
    unsupervise_if_last(Pid, Waiting, Ctx). 

handle_avail_timeout({timeout, Timer, {aquire, Pid, Ref}}, 
...
handle_noavail_timeout({timeout, Timer, {aquire, Pid, Ref}}, 
...
remove_user(Pid, Ctx=#ctx{waiting = Waiting, refs = Refs}) ->
    NewWaiting = 
    ...
    NewRefs = 
	lists:fold(fun({{P, _Ref}, _}, Acc) when P =:= Pid -> Acc;
		       (Other, Acc) ->
			    Acc ++ [Other]
		   end, [], Refs),
    Ctx#ctx{waiting = NewWaiting, refs = NewRefs}.
